...
class ProveedorController extends Controller
{
    ...
    {
        $model = new Proveedor();
        $agregar = [new MatProvAdquirido];

        if ($model->load(Yii::$app->request->post()) && $model->save()) {

            $agregar = Model::createMultiple(SactMatConsume::classname());
            Model::loadMultiple($agregar, Yii::$app->request->post());



            // validate all models
            $valid = $model->validate();
            $valid = Model::validateMultiple($agregar) && $valid;

            if ($valid) {
                $transaction = \Yii::$app->db->beginTransaction();
                try {
                    if ($flag = $model->save(false)) {
                        foreach ($agregar as $agregar) {
                            $agregar->SACT_ID = $model->SACT_ID;
                            if (! ($flag = $agregar->save(false))) {
